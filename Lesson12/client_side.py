...
class Client(Thread, metaclass=ClientVerifier):
    def __init__(self, cl_db, func=None, sock=None, name=None, type_cl=None):
        super().__init__()
        self.daemon = True
        self.func_targ = func
        self.socket = sock
    ...
    def run(self):
        if self.type_cl == 'rcv':
            ...
            self.func_targ(self.socket, self.username, self.cl_db)
...
def ans_from_users(message):
    if 'action' in message and message['action'] == 'message' and \
        ...
        print(f'Ответ сервера: >>> {message["response"]}')
    ...
    while True:
        try:
            ...
            if msg_of_auth['to_users']:
                cl_db_1.store_message_in_db(msg_of_auth['author'],
                                            msg_of_auth['send_to'],
                                            msg_of_auth['post_text'])
                send_msg(to_chat_sock, msg_of_auth)
            elif msg_of_auth['post_text'] == '@create@':
                if cl_db_1.create_contact(msg_of_auth['send_to']):
                    send_msg(to_chat_sock, msg_of_auth)
            else:
                send_msg(to_chat_sock, msg_of_auth)
...
def main_start():
    ...
    client1 = Client(cl_db1, func=None, sock=None, name=username, type_cl='rcv')
    ...
    client2 = Client(cl_db1, func=None, sock=None, name=username, type_cl='snd')
