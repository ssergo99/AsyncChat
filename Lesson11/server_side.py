                ...
                for client_in_r in r:
                    try:
                        ...
                        if new_msg['action'] == 'presence':
                            self.users_in_chat[new_msg['user']] = client_in_r
                            cl_ip, cl_port = client_in_r.getpeername()
                            self.db.user_login(new_msg['user'], cl_ip, cl_port)
                            ...
                            if new_msg['logout']:
                                ...
                                msg_to_send = {
                                    ...
                                    'post_text': str(list_of_users_str),
                                }
                                self.msg_list.append(msg_to_send)
                            elif new_msg['post_text'] == '@logins@':
                                all_logins_list = []
                                all_logins = db1.get_list_logins(new_msg['author'])
                                ...
                                contacts = db1.get_list_connections(new_msg['author'])
